...
class WeatherData(BaseModel):
    ...
    event_timestamp: datetime = Field(
        ..., description="Time when the measurement occurred"
    )
    source: str = Field(
        ..., description="Data source identifier", example="open-meteo-api"
    )
    unit_system: UnitSystem = Field(
        default=UnitSystem.metric, description="Unit system used"
    )
...
class ProcessedWeatherData(WeatherData):
    ...
    is_heatwave: bool = Field(
        ..., description="Indicates if temperature exceeds threshold"
    )
